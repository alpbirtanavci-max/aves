...
test('index R15D rc3.9.21 onay özeti düzeltme sürümü', index.includes('R15D-RC3.9.21</b>'));
test('app R15D rc3.9.21 onay özeti düzeltme sürümü', app.includes("const APP_VERSION = 'R15D-rc3.9.21'"));
...
test('her fotoğraf kategorisinde denetçiye yönlendirme metni var', app.includes("'kuyu_dibi', 'Kuyu Dibi', 'Tampon kaideleri") && app.includes('capture="environment" multiple'));
...
test('eski cihaz fotoğrafları IndexedDB yükseltmesinde kategoriye dönüştürülüyor', app.includes('LEGACY_FOTOGRAF_KATEGORISI') &&
  app.includes('LEGACY_FOTOGRAF_KATEGORISI[foto.madde_id]') &&
  app.includes('foto.kategori = LEGACY_FOTOGRAF_KATEGORISI[foto.madde_id]'));
test('rc3.9.21 geçiş migrationı sunucu fotoğraflarını silmiyor, eski alanları geri nullable ekliyor',
...
test('rc3.9.21 tetikleyicisi eski dokuz maddeyi kayıpsız kategoriye eşliyor',
  ['MAD-0006','MAD-0072','MAD-0110','MAD-0111','MAD-0162','MAD-0364','MAD-0366','MAD-0368','MAD-0369']
    .every(id => rc3921PhotoCompatibilityMigration.includes(`'${id}'`)) &&
  rc3921PhotoCompatibilityMigration.includes('denetim_fotografi_legacy_kategori_ata') &&
  rc3921PhotoCompatibilityMigration.includes('Fotoğraf kategorisi belirlenemedi'));
